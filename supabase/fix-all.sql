-- ============================================================
-- XI TJKT 1 Class Portal — FIX ALL (idempotent, safe to re-run)
-- Supabase Dashboard → SQL Editor → Run
-- ============================================================

-- ── 1. BUAT / PERBAIKI TABEL member ─────────────────────────
...
-- Tambahkan absen_number jika belum ada (aman jika sudah ada)
ALTER TABLE public.member
  ADD COLUMN IF NOT EXISTS absen_number INTEGER NOT NULL DEFAULT 0;

-- Isi absen_number dari sort_order untuk baris yang masih 0
UPDATE public.member
SET absen_number = sort_order
WHERE absen_number = 0 AND sort_order > 0;

-- Fallback: beri nomor urut otomatis untuk yang masih 0
...
CREATE INDEX IF NOT EXISTS idx_member_sort  ON public.member(sort_order);
...
-- ── 5. BUAT TABEL gallery_album + gallery_photo ──────────────
...
-- ── 6. BUAT TABEL app_settings ──────────────────────────────

CREATE TABLE IF NOT EXISTS public.app_settings (
  id                      INTEGER     PRIMARY KEY DEFAULT 1 CHECK (id = 1),
  theme                   TEXT        NOT NULL DEFAULT 'dark-navy'
                            CHECK (theme IN ('dark-navy', 'dark-slate', 'pure-black', 'glass-blur', 'glass', 'light')),
...
-- Kolom logo tambahan (tambahkan jika belum ada)
ALTER TABLE public.app_settings ADD COLUMN IF NOT EXISTS logo_header      TEXT NOT NULL DEFAULT '';
ALTER TABLE public.app_settings ADD COLUMN IF NOT EXISTS logo_favicon     TEXT NOT NULL DEFAULT '/favicon.svg';
ALTER TABLE public.app_settings ADD COLUMN IF NOT EXISTS logo_admin       TEXT NOT NULL DEFAULT '';
ALTER TABLE public.app_settings ADD COLUMN IF NOT EXISTS logo_placeholder TEXT NOT NULL DEFAULT '/hu-tao-placeholder.png';
ALTER TABLE public.app_settings ADD COLUMN IF NOT EXISTS brand_title      TEXT NOT NULL DEFAULT 'XI TJKT 1';
ALTER TABLE public.app_settings ADD COLUMN IF NOT EXISTS brand_subtitle   TEXT NOT NULL DEFAULT 'SMKN 1 BOYOLALI';

-- Update CHECK constraint tema (hapus lama → tambah baru, aman jika belum ada)
DO $$
DECLARE
  con_name TEXT;
BEGIN
  ...
  IF con_name IS NOT NULL THEN
    EXECUTE format('ALTER TABLE public.app_settings DROP CONSTRAINT %I', con_name);
  END IF;
...
-- Storage policies (hapus dulu biar aman di-run ulang)
...
CREATE POLICY "Auth upload member photos"   ON storage.objects FOR INSERT  TO authenticated WITH CHECK (bucket_id = 'member-photos');
CREATE POLICY "Auth update member photos"   ON storage.objects FOR UPDATE  TO authenticated USING  (bucket_id = 'member-photos');
CREATE POLICY "Auth delete member photos"   ON storage.objects FOR DELETE  TO authenticated USING  (bucket_id = 'member-photos');

CREATE POLICY "Public read gallery photos"  ON storage.objects FOR SELECT                  USING  (bucket_id = 'gallery-photos');
CREATE POLICY "Auth upload gallery photos"  ON storage.objects FOR INSERT  TO authenticated WITH CHECK (bucket_id = 'gallery-photos');
CREATE POLICY "Auth update gallery photos"  ON storage.objects FOR UPDATE  TO authenticated USING  (bucket_id = 'gallery-photos');
CREATE POLICY "Auth delete gallery photos"  ON storage.objects FOR DELETE  TO authenticated USING  (bucket_id = 'gallery-photos');
